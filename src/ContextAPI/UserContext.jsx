...
export function Usercontextprovoider({children})
{
    ...
    async function Signup(uname,email,password)
    {
        ...
        const UsernameExist=await IsUsernameExist(uname)
        ...
        if(UsernameExist)
        {
            ...
            toast({
                title: 'Sign up',
                description: "Username is alredy taken",
                status: 'error',
                duration: 9000,
                position:"top-right",
                isClosable: true,
    ...
    useEffect(()=>
 {
    ...
    return <Usercontext.Provider value={{Signin,Signup,user,userdata,Logout,usernametaken}}>{children}</Usercontext.Provider>
}
